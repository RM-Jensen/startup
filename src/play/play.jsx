...
export function Play() {
    ...
    const [gameInfo, setGameInfo] = useState('Set a stake and click deal to play!');
    const [gameStake, setGameStake] = useState('');
    const [houseCardDisplay, setHouseCardDisplay] = useState('');
    ...
    class Game {
        ...
        startGame() {
            ...
            if (!gameStake) {
                setGameInfo('Set a stake to start!');
                return;
            }
            //todo: don't let them stake more than their balance, load balances as soon as page is opened
            if (this.getPlayerName() == null) {
                setGameInfo('Log in to play!');
                return;
            }
           /* if (localStorage.getItem(this.getPlayerName()) < gameStake //todo: this check is broken
                    && gameStake > 1) {
                document.querySelector('#gameInfo').innerHTML = '<p>You must set the stake to less than your balance (or 1)!</p>';
                return;
            } */
            this.broadcastEvent(this.getPlayerName(), GameStartEvent, gameStake);
            //clean up images from last game, if present
            setHouseCardDisplay(''); //remove house card images
            setPlayerCardDisplay('');  //remove player card images
    
            setGameLive(true);
            this.house.push(this.deal());
            this.house.push(this.deal());
            this.showHouseCards();
            setGameInfo('Good luck!');
        ...
        showHit(dealt) {
            ...
            setPlayerCardDisplay(playerCardDisplay +  `<img src="${cardImage}"/>`);
        }
     
        showHouseCards() {
            setHouseCardDisplay('<img id="houseCard1" alt="houseCard1" src="card_back.png"/>' +
        ...
        declareWinner(playerWon, playerActiveParty, message, tieGame = false) {
            ...
            setGameInfo(`${activeParty}${message}</p><p>Use the deal button to play again!`);
        ...
        showStand() {
            ...
            setHouseCardDisplay(`<img id="houseCard1" alt="houseCard1" src="${house1}"/> 
                                    <img id="houseCard2" alt="houseCard2" src="${house2}"/>`);
        }
    
        showDealerHit(dealt) {
            var cardImage = this.getCardImg(dealt);
            setHouseCardDisplay(houseCardDisplay + `<img src="${cardImage}"/>`);    
        }
      ...
      displayMsg(cls, from, msg) {
        setPlayerMessages(`<div class="event"><span class="${cls}-event">${from}</span> ${msg}</div>` + playerMessages);
      }
    ...
    return (
        <main className='container-fluid bg-secondary text-center'>
            <div id="firstRowItems">
                ...
                <form id="stakeForm" method="post" onSubmit={() => false}>
                    <label htmlFor="stake">Stake ($): </label>
                    <input type="number" name="stake" id="stake" min="1" step="1" disabled={gameLive}/> {/*might need to tamper with*/}
